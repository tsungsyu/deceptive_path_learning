...
class GameState:
  ...
  def getObserverReward(self):
    ...
    reward = self.calculateObserverReward()
    return reward

  def calculateObserverReward(self):
    probOfTrueGoal = self.data.statePossibility[self.data.trueGoal]
    probOfDummyGoals = {key:prob for key, prob in self.data.statePossibility.items()
                        if key != self.data.trueGoal}
    maxOfDummyGoal = max(probOfDummyGoals.values())
    reward = math.sin(maxOfDummyGoal - probOfTrueGoal)
